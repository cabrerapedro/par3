...
| Messaging | WhatsApp via **Kapso** (managed WhatsApp Business Platform: SDK, Broadcasts API, webhooks) | Automated campaigns, reactivation, and lesson reminders. Requires Kapso account, dedicated number, approved templates, opt-in |
| Calendar | Google Calendar integration | Read/sync the instructor's existing agenda; do not rebuild a booking engine |
