...
forat.golf es un **copiloto de práctica de golf** para instructores profesionales y sus alumnos, que se está extendiendo a una **herramienta simple para operar la escuela de un instructor independiente**.
...
Alrededor de ese core se agregan módulos de gestión **simples y útiles desde el día 1**: contactos de alumnos, campañas y reactivación, agenda/operación, y una experiencia del alumno con su journey y el registro de cada clase. La regla de estos módulos: simple para desarrollar rápido, útil de entrada, y usar AI para automatizar workflows o generar una buena experiencia — nunca avanzado ni complejo. Lo genérico se integra (ej. Google Calendar), no se reconstruye.
...
**Modelo de negocio:** En definición junto a Steve, que pasa a ser **co-founder**. El objetivo inmediato es hacer crecer su escuela. El alumno siempre accede gratis. (Cualquier comisión por clases la maneja Pedro por fuera del producto.)
...
| Email | Resend |
| Mensajería | WhatsApp vía **Kapso** (WhatsApp Business Platform gestionada: SDK, Broadcasts API, webhooks) — envío automático de campañas/recordatorios |
| Calendario | Integración con Google Calendar (no se reconstruye) |
| i18n | next-intl |
...
**Módulos de escuela (versión simple — ver `08-MODULOS-ESCUELA.md`):**
- Contactos: carga e import de los alumnos de Steve (nombre, teléfono/WhatsApp, nivel, notas, estado activo/dormido)
- Campañas y reactivación: segmentar + mensaje hiperpersonalizado por AI, **envío automático vía WhatsApp (Kapso)** (plantilla aprobada con variables para el primer toque; texto libre por AI dentro de la ventana de 24h) y email. Requiere cuenta Kapso, número dedicado, plantillas aprobadas y opt-in.
- Agenda/operación: integrar Google Calendar y ponerle contexto de alumno; grupos implícitos por día+horario; recordatorios y resumen semanal auto-redactados
- Experiencia del alumno: app con su journey (templates ajustables por alumno) + registro de cada clase; resúmenes generados por AI

### Fuera del MVP / NO ahora

- Pagos y billing del alumno (lo maneja el campo, por fuera)
- Motor de reservas propio (se integra Google Calendar, no se reconstruye)
- Concierge/captación de leads automatizada (la puerta de entrada la maneja Steve por ahora)
- Capa de consistencia entre instructores (no necesaria en este punto)
- Multi-instructor (vendrá cuando crezca la escuela; sin capa de consistencia por ahora)
- Notificaciones push
...
11. Los módulos de escuela son simples y útiles desde el día 1 — nunca avanzados ni complejos. Si algo genérico existe (calendario, pagos), se integra, no se reconstruye.
12. AI se usa para automatizar workflows administrativos y generar una buena experiencia del alumno, no para reemplazar el criterio del instructor.
